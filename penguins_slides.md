...
```
(표 내용: tables/crosstab_species_island.md 파일 확인)
```

**인사이트:**

교차표는 각 섬에서 관찰된 종의 빈도를 정리해 주며, 섬-종 조합의 분포 패턴을 정량적으로 보여 줍니다. 절대빈도를 비율로 변환하면 표본 수 차이를 보정한 비교가 가능하고, Chi-square 검정 등으로 분포 차이의 통계적 유의성을 확인할 수 있습니다. 예를 들어 한 섬에서 특정 종이 현저히 우세하다면 그 섬의 생태적 조건이 해당 종에 우호적일 가능성이 높습니다.
...
```
(표 내용: tables/pivot_island_species_bodymass.md 파일 확인)
```

**인사이트:**

피봇테이블은 섬별·종별 평균 체질량을 비교하여 지역적 차이와 종 내 변동을 동시에 파악하게 합니다. 동일 종이라도 섬 간 평균 체질량 차이는 먹이 자원, 번식 상태, 계절적 요인 등 환경적 스트레스의 차이를 반영할 수 있습니다. 차이의 통계적 검증을 위해 표준편차 또는 표준오차를 함께 제시하고, 필요 시 다중비교 보정을 통해 유의미한 차이를 확인해야 합니다.
